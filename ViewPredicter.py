...
def read_view_data(path):
    ...
    for i in tqdm(range(len(file_list))):
        ...
        if ViewDataGetter.check_platform(data['video_id']) == 'niconico':
            platform = 1
        temp_x.append(platform)                         #x[0]...Platform; YouTUbeなら0, ニコ動なら1
        temp_x.append(str2int(data['subscriberCount']))          #x[1]...チャンネル登録者数、あるいはフォロワー数
        temp_x.append(data['length'])                   #x[2]...動画の長さ [s]
        temp_x.extend(genre_convertor(data['genre']))   #x[3]...動画のジャンル
        for i in data['view_data']:
            add_x.extend(temp_x)
            add_x.append(i[0])                   #x[-1]...動画の投稿からの経過時間 [h]
...
training_nnw(x_train, y_train, './nnw', [100,20], 1000)
